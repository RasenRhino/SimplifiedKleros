...
/// @title SimpleKlerosPhase2 - Staking-Based Weighted Voting
/// @notice Phase 2: Voting weight based on STAKED tokens (locked capital), commit-reveal scheme, no redistribution yet
contract SimpleKlerosPhase2 {
    // --- Types ---
    ...
    // --- Juror staking (PHASE 2 KEY FEATURE) ---
    ...
    function drawJurors(uint256 id) external {
        ...
        while (d.jurors.length < jurorsPerDispute) {
            ...
            // PHASE 2: Check if juror has sufficient stake and is not already selected
    ...
    /// @dev PHASE 2 KEY FEATURE: Voting weight = juror's STAKED amount
    function revealVote(uint256 id, uint8 vote, bytes32 salt) external {
        ...
        // PHASE 2 KEY FEATURE: Weight is based on STAKED amount (not balance)
    ...
    /// @dev PHASE 2: No stake redistribution yet - just unlock jurors
    function finalize(uint256 id) external {
        ...
        // Unlock juror stakes (NO redistribution in Phase 2)
